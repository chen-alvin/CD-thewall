...
CREATE TABLE IF NOT EXISTS `thewall`.`messages` (
  ...
  CONSTRAINT `fk_messages_users`
    FOREIGN KEY (`user_id`)
    REFERENCES `thewall`.`users` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
AUTO_INCREMENT = 18
...
CREATE TABLE IF NOT EXISTS `thewall`.`comments` (
  ...
  CONSTRAINT `fk_comments_messages1`
    ...
    REFERENCES `thewall`.`messages` (`message_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
insert into users (user_id,first_name, last_name, email, password, created_at)
values (1,'Alvin','Chen', 'terrier150@gmail.com', '$2b$12$PecGisyckudEk/Tgv7dPbOSwB6OmrB8OSpQxD9SnTE6SSKJh7poWa', NOW());
